...
ratings = {
    ...
    "Inception": [8.0,7.0, 7.5],
    "Jurassic Park": [7.5, 7.0, 8.0],
    "The Matrix": [8.8, 9.2, 8.5],
}

#Função para calcular a média de avaliações de um filmes
avarege_rating = lambda movie_name: sum(ratings[movie_name]) / len(ratings[movie_name])

#Função que verifica se um filme está na lista
check_movie = lambda movie_name: movie_name in moviesList

#Função para recomendar um filme em base a avaliação média
recommend_movie = lambda movie_name: f"Recomendo assistir {movie_name} com média de {avarege_rating(movie_name):.2f}"

print(f"Média de avaliação do filme The Matrix: {avarege_rating("The Matrix")}")
